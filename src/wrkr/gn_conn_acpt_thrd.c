...
gn_conn_acpt_thrd (void * const p)
{
    ...
    atomic_store_explicit (&data->state, GN_CONN_ACPT_THRD_STATE_RUNNING, memory_order_release);
    ...
    while (main_loop)
    {
        if (atomic_load_explicit (&data->stop, memory_order_relaxed))
        {
            ...
            atomic_store_explicit (&data->state, GN_CONN_ACPT_THRD_STATE_STOPPING, memory_order_release);
            main_loop = false;
            break;
        }
    ...
    atomic_store_explicit (&data->state, GN_CONN_ACPT_THRD_STATE_STOPPED, memory_order_release);
    return NULL;
}